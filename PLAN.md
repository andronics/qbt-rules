...
- type: notify
  params:
    ...
    message: "[${rule.name}] ${info.name} matched, ratio ${info.ratio}, tags: ${info.tags}"
```

New `notifications:` config section (`webhook_url` with `_FILE` secret support, `service`). `ActionExecutor` does **not** hold a `Config` reference — `_FILE` resolution only happens via `cli.py`'s `resolve_config()`, which needs CLI `args` that `ActionExecutor` never has. Instead, `get_notifications_config(args, config_obj)` resolves the webhook URL/service once at server startup (same place `server_config`/`schedule_entries` already are) and threads the plain resolved dict down through `Worker` → `RulesEngine` → `ActionExecutor` as an optional `notifications_config` param. Always-fire, no idempotency tracking (matches `reannounce`/`recheck`); accepted risk that a rule re-evaluated across multiple contexts will re-notify — documented, with the `add_tag` + condition-exclusion workaround. Confirmed via a real end-to-end test (not mocked) that `notify` placed after `delete_torrent` in the same rule still renders the correct torrent name.

**Update (message templating: `${rule.*}` and `${info.*}`/`${trackers.*}`/etc, replacing `{field}`)**: The original `{name}`/`{ratio}`/`{tags}` `str.format()`-based templating only ever saw the torrent dict, with no way to reference the rule itself (its name, context, etc) and no access to `trackers.*`/`files.*`/`peers.*` collection fields conditions can already use. Replaced with a `${namespace.field}` convention split across two resolution phases:

- `${vars.x}` (existing) and `${rule.x}` (new — e.g. `${rule.name}`, `${rule.context}`) resolve once, at config-load time, in `resolver.py`. `${rule.*}` is self-referential (resolves against the rule currently being processed, not a shared table like `refs.vars`) — `RuleResolver._substitute_tokens()` (renamed from `_substitute_vars`, now takes the rule as a second argument) dispatches `rule.`-prefixed paths to a new `_resolve_rule_field()`, mirroring `_resolve_var()`'s format/lookup error handling (`InvalidRuleFieldError`/`UnknownRuleFieldError`, new in `errors.py`). Its `TOKEN_PATTERN` regex is deliberately scoped to only `vars.`/`rule.` prefixes, and a whole-string token check now looks at the prefix (`RUNTIME_TOKEN_NAMESPACES`) before dispatching, rather than blindly assuming any `${...}`-shaped whole string is a `vars.` reference — which fixes a latent bug where a whole-string `${info.x}` (or any other runtime token) would have incorrectly raised `InvalidVariableError` at load time before this existed.
- `${info.x}`, `${trackers.x}`, `${files.x}`, `${peers.x}`, `${properties.x}`, `${webseeds.x}`, `${transfer.x}`, `${app.x}` resolve at runtime, per torrent, in `engine.py` — the same dot-notation endpoints `_get_field_value()` already routes for conditions. Made public as `ConditionEvaluator.get_field_value()` and shared with `ActionExecutor` via a new optional `field_resolver` constructor param (`RulesEngine` wires its own evaluator in; falls back to a private, unshared `ConditionEvaluator` otherwise, so all ~150 existing `ActionExecutor(mock_api, ...)` test call sites keep working unchanged) — this means `${trackers.url}`/`${files.name}` in a notify message reuse the same lazy-loaded, per-run cache a condition on the same torrent already populated, instead of re-fetching from the qBittorrent API. Collection results (a torrent with several trackers/files, or `${info.tags}`) are comma-joined. An unrecognized *namespace* (e.g. a typo'd prefix) still skips the notification with a warning, matching the old strict-on-typo behavior; an unrecognized *field* within a real namespace (e.g. `${info.does_not_exist}`, or `${trackers.url}` on a torrent with none) renders as an empty string instead, since "missing key" vs. "torrent legitimately has none" can't be cleanly distinguished across all these endpoint shapes.

`advanced-rules-example.yml`'s closing rule now demonstrates both phases together in one message. Verified end-to-end through the real file/resolver/engine (not just unit-mocked): `${rule.name}` shows up already-substituted in the *resolved* rule (config-load time), `${info.name}`/`${info.ratio}` remain as literal placeholders until rendered per-torrent at runtime.
...
**Update (session-based dashboard auth)**: The `?key=` in every dashboard URL was itself a problem (leaks into browser history, referrer headers, screen-share screenshots) independent of the earlier login-form work. `/login` is now `GET`+`POST`: the form submits the key via POST body (never appears in the URL/history), and on success sets a signed Flask session cookie (`session['authenticated'] = True`, `session.permanent = True`) that `require_api_key_dashboard` checks on every subsequent request before falling back to requiring `?key=`/header again. `app.secret_key` (signs the session cookie) is derived via `sha256("qbt-rules-dashboard-session:" + api_key)` rather than a fresh random value per process -- Gunicorn's `server.workers > 1` forks multiple independent `create_app()` calls, and a per-process random secret would make a cookie signed by one worker fail to validate on another; hashed rather than reused as-is so the raw API key never doubles as the cookie-signing key. Every internal link (`base.html` nav, job/rule detail links, pagination, "View all" links) dropped its `key=api_key` `url_for()` argument -- `api_key` is no longer threaded into any template context at all, since nothing references it anymore. A direct `?key=`/header link still works as a one-shot bootstrap (and refreshes the session) for anyone who wants to skip the form. `_sanitize_next()` (reduces an arbitrary `next` value to a same-origin path+query, discarding scheme/host and any embedded `?key=`) is applied both when rendering the login form's hidden field and again right before the post-login redirect, since the hidden field is still client-controlled input by the time it comes back in the POST body -- verified live that a crafted `next=https://evil.example/steal` redirects to `/steal`, not the external host.
