...
**Status**: In progress. Eight initiatives, all confirmed in scope for a single v0.6.0 release (not staged across multiple minors). Initiatives 6 (Prometheus metrics) and 7 (legacy cleanup, including the `keep_files` removal originally scoped for v0.7.0) were both folded in here since nothing had been tagged/released yet — no reason to split either into a separate cycle. Initiative 8 (the `_LOG`/`_LOGGING` env var fix) surfaced during Initiative 7's legacy audit as the same bug shape found for `qbittorrent.*`, applied to `logging.*`.
